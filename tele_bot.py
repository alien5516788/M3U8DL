...
def start(message):
   ...
   #check the session index (current user)
   if message.from_user.id == funcs.user_id:
      if funcs.session_status == True:
...
---If you need to terminate the process, Press /start .
---To clear request, press /clear .''')


   #check the session index (new user)
   else:
